...
namespace RavenFS.Tests.RDC
{
	...
	public class SynchronizationTestsAfterChanges : MultiHostTestBase
	{
		...
		public void Synchronize_file_with_different_beginning(int size)
		{
			...
			sourceClient.Synchronization.SynchronizeDestinationsAsync("test.txt").Wait();
			...
			using (var resultFileContent = new MemoryStream())
			{
				...
				resultFileContent.Position = 0;
				var sr = new StreamReader(resultFileContent);
				resultSeed = sr.ReadToEnd();
			...
			var sr2 = new StreamReader(sourceContent);
			var sourceContentString = sr2.ReadToEnd();
